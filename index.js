...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/") {
    ...
    for (let index = 0; index < 6; index++) {
      allTheBicyles += replaceTemplate(AllMainBicycles, bicycles[index]);
    }
    html = html.replace(/<%AllMainBicycles%>/g, allTheBicyles);
...
function replaceTemplate(html, bicycle) {
  ...
  html = html.replace(/<%DESCRIPTION%>/g, `${bicycle.desc}`);
